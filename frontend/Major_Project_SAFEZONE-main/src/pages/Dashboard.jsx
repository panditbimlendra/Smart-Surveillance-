...
function Dashboard() {
  ...
  const [cameraId, setCameraId] = useState("main_lobby");
  const [cameraSource, setCameraSource] = useState(
    "rtsp://admin:password@192.168.1.100:554/Streaming/Channels/101",
  );
  const [cameraActionState, setCameraActionState] = useState("");
  const [webcamId, setWebcamId] = useState("laptop_webcam");
  const [webcamDeviceId, setWebcamDeviceId] = useState(0);
  const [webcamActionState, setWebcamActionState] = useState("");
  ...
  async function refreshOperationalData() {
    const [alertsData, logsData, streamData, webcamData] = await Promise.all([
      getRecentAlerts(12),
      getRecentLogs(20),
      getStreamStatus(),
      getWebcamStatus(),
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        const [healthData, alertsData, logsData, streamData, webcamData] =
          await Promise.all([
            getHealth(),
            getRecentAlerts(12),
            getRecentLogs(20),
  ...
  const streamPreviewUrl = useMemo(() => {
    ...
    return getStreamFrameUrl(
      `${streamStatus.frames_processed}-${streamStatus.camera_id || "cam"}`,
    );
  }, [streamStatus]);
  ...
  async function handleStartWebcam() {
    try {
      ...
      setWebcamActionState("Starting webcam...");
      await startWebcam({
        device_id: parseInt(webcamDeviceId, 10) || 0,
        camera_id: webcamId,
      });
      const status = await getWebcamStatus();
      setWebcamStatus(status);
      setWebcamActionState(
        status.streaming
          ? `Webcam started: ${status.camera_id || webcamId}`
          : status.error_message || "Webcam did not stay connected",
      );
    } catch (webcamError) {
      setWebcamActionState(webcamError.message || "Could not start webcam");
    } finally {
  ...
  async function handleStopWebcam() {
    try {
      ...
      setWebcamActionState("Stopping webcam...");
      await stopWebcam();
      const status = await getWebcamStatus();
      setWebcamStatus(status);
      setWebcamActionState("Webcam stopped");
    } catch (webcamError) {
      setWebcamActionState(webcamError.message || "Could not stop webcam");
    } finally {
  ...
  async function handleAnalyzeUpload() {
    ...
    try {
      ...
      const detectionLabel = audioOnlyDetection
    ...
    >
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2 text-sm text-mono text-slate-700">
          <span
            className={`w-1.5 h-1.5 rounded-full ${health?.status === "ok" ? "bg-emerald-500" : "bg-red-500"}`}
          />
      ...
      <div className="grid xl:grid-cols-2 gap-4 mb-6">
              ...
              <div className="text-center px-6">
                ...
                {streamStatus?.last_result && (
                  <p className="text-sm text-sky-700 mt-2 font-mono">
                    {streamStatus.last_result.risk_level} ·{" "}
                    {streamStatus.last_result.top_video}
                  </p>
                )}
              </div>
            )}
          ...
          <div className="p-5 space-y-4 bg-white">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                ...
                <p className="text-lg text-slate-900 font-semibold">
                  {Number(streamStatus?.last_result?.risk_score ?? 0).toFixed(
                    4,
                  )}
                </p>
              </div>
        ...
        {streamStatus?.abnormal_frame_url && (
          <div className="glass-card overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200">
              <div className="flex items-center gap-2">
                <AlertCircle className="w-4 h-4 text-red-600" />
                <span className="text-sm text-slate-900 font-semibold">
                  Abnormal Activity Frame
                </span>
              </div>
              <span className="text-xs font-mono text-red-600">
                {streamStatus?.abnormal_frame_ts
                  ? new Date(
                      streamStatus.abnormal_frame_ts * 1000,
                    ).toLocaleTimeString()
                  : "Captured"}
              </span>
            </div>
            <div className="p-4 bg-white">
              <img
                src={`http://localhost:8000${streamStatus.abnormal_frame_url}`}
                alt="Abnormal Activity Frame"
                className="w-full rounded-lg border border-slate-200 max-h-80 object-contain"
                onError={() =>
                  console.warn("Failed to load abnormal frame image")
                }
              />
              <p className="text-xs text-slate-600 mt-2">
                ...
                risk level ({streamStatus?.last_result?.top_video})
              </p>
            </div>
          </div>
        )}
      ...
      <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4 mb-6">
        <div className="glass-card p-5">
          ...
          <div className="grid sm:grid-cols-2 gap-3 mb-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-sky-400"
                placeholder="rtsp://admin:password@IP:554/Streaming/Channels/101"
              />
            </div>
          </div>
          ...
          <p className="mt-3 text-sm text-slate-600">
            {cameraActionState ||
              "Enter CCTV RTSP URL and click Start Camera to begin live detection."}
          </p>
        </div>
        ...
        <div className="glass-card p-5">
          ...
          <div className="grid sm:grid-cols-2 gap-3 mb-4">
            <div>
              ...
              <input
                value={webcamId}
                onChange={(event) => setWebcamId(event.target.value)}
                className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-sky-400"
                placeholder="laptop_webcam"
              />
            </div>
            <div>
              <label className="block text-sm text-slate-600 mb-1">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            >
              ...
              {isStartingWebcam ? "Starting..." : "Start Webcam"}
            </button>
            <button
              onClick={handleStopWebcam}
              disabled={isStoppingWebcam}
              className="inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-800 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60"
            >
              <Square className="w-4 h-4" />
              {isStoppingWebcam ? "Stopping..." : "Stop Webcam"}
            </button>
          </div>

          <p className="mt-3 text-sm text-slate-600">
            {webcamActionState ||
              "Use device index 0 for your default laptop webcam (usually built-in)."}
          </p>
        </div>
        ...
        <div className="glass-card p-5">
          ...
          <div className="space-y-3">
            ...
            <p className="text-sm text-slate-600">
              {analyzeState ||
                "Upload a media file for direct backend detection."}
            </p>
            ...
            {analyzeResult && (
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-800 space-y-2">
                {detectionSummary && (
                  <div
                    className={`rounded-xl border px-3 py-2 text-center text-sm font-semibold tracking-widest ${detectionSummary.tone}`}
                  >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold">
                    {analyzeResult.fusion?.risk_level}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-500">Risk score</span>
                  <span className="font-mono">
                    {Number(analyzeResult.fusion?.risk_score ?? 0).toFixed(4)}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-500">Top video</span>
                  <span className="font-mono">
                    {analyzeResult.fusion?.top_video}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-500">Top audio</span>
                  <span className="font-mono">
                    {analyzeResult.fusion?.top_audio}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-500">Frames sampled</span>
                  <span className="font-mono">
                    {analyzeResult.frames_sampled}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-500">Audio available</span>
                  <span className="font-mono">
                    {analyzeResult.audio_available ? "yes" : "no"}
                  </span>
                </div>
                {analyzeResult.duration_secs != null && (
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-mono">
                      {Number(analyzeResult.duration_secs).toFixed(2)} s
                    </span>
                  </div>
                )}
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono">
                    {analyzeResult.processing_ms} ms
                  </span>
                </div>
                {Array.isArray(analyzeResult.audio_detections) &&
                  analyzeResult.audio_detections.length > 0 && (
                    <div className="pt-3 border-t border-slate-200">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-slate-500">Audio detections</span>
                        <span className="font-mono">
                          {analyzeResult.audio_detections.length}
                        </span>
                      </div>
                      <div className="space-y-2">
                        {analyzeResult.audio_detections
                          .slice(0, 6)
                          .map((detection, index) => (
                            <div
                            ...
                            >
                              <div className="flex items-center justify-between gap-3">
                                ...
                                <span className="font-mono text-xs">
                                  {(Number(detection.confidence) * 100).toFixed(
                                    1,
                                  )}
                                  %
                                </span>
                              ...
                              <div className="mt-1 text-xs text-slate-500 font-mono">
                                {Number(detection.start_time).toFixed(1)}s -{" "}
                                {Number(detection.end_time).toFixed(1)}s{" · "}
                                {detection.severity}
                              </div>
                            </div>
                          ))}
